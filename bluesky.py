...
def generate_line_chart(client, db_path, title, total_sightings):
    ...
    # Plot the line graph
    plt.figure(figsize=(8, 8), dpi=500)  # Square figure size (8x8 inches)
    ...
    with tempfile.NamedTemporaryFile(suffix='.png', delete=False) as tmp_file:
        try:
            plt.savefig(tmp_file.name, bbox_inches='tight', pad_inches=0.1, dpi=500)  # Add 0.1 inches of padding
            plt.close()
